...
int main(int argc, char ** argv)
{
   ...
   P = new pakissat();
   P -> infile = Parameters::getFilename();
   P -> thread_num = cpus;
   P -> read_var_clause_num(P -> infile.c_str());
   P -> alloc_init();
   P -> readfile(P -> infile.c_str()); 
   // Create and init solvers
   std::vector<SolverInterface *> solvers;

   SolverFactory::createLstechSolvers(cpus, solvers);
