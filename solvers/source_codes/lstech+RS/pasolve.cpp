...
void do_simplify(string file, string spath, string sfile) {
    string info = string("./seqsimple ") + file + string(" ") + spath + string(" > ") + sfile;
    system(info.c_str());
}
...
void* seqsolve(void *arg) {
    ...
    while (!paS->globalEnding) {
        ...
        if (thread_id) {
            paS->T[thread_id].solver -> initshuffle = thread_id;
        }
