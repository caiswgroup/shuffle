...
void
MapleCOMSPSSolver::setBumpVar(int v) {
   // bump_var = v;
}

void 
MapleCOMSPSSolver::addOriginClauses(simplify *S) {
   vec<Lit> lits;
   while (S->vars - 1 >= solver->nVars()) solver->newVar();
   for (int i = 1; i <= S->clauses; i++) {
      int l = S->clause[i].size();
      for (int j = 0; j < l; j++) 
         lits.push(MINI_LIT(S->clause[i][j]));
...
void
MapleCOMSPSSolver::initshuffle(int id)
{
}
void
...
void 
MapleCOMSPSSolver::setParameter(parameter p)
{
...
void
MapleCOMSPSSolver::addClauses(const std::vector<ClauseExchange *> & clauses)
{
...
void
MapleCOMSPSSolver::addInitialClauses(const std::vector<ClauseExchange *> & clauses)
{
...
void
MapleCOMSPSSolver::addLearnedClauses(const std::vector<ClauseExchange *> & clauses)
{
...
void
MapleCOMSPSSolver::getLearnedClauses(std::vector<ClauseExchange *> & clauses)
{
