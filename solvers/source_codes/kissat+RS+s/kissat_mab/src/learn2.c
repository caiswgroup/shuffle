...
kissat_importClauses(kissat *solver)
{
  ...
  while ((res = solver->cbkImportClause(solver->issuer, &lbd, solver->importedClause)) != -1) {
    // printf("c import Clause: size %d\n", solver->importedClause->sz);
    if (res == -10) {
    ...
    if (solver->importedClause->sz == 1) {
      // printf("learn unit %d\n", solver->importedClause->data[0]);
      kissat_assign_unit(solver, solver->importedClause->data[0]);
    }
    else {
      // printf("learn binary %d %d\n", solver->importedClause->data[0], solver->importedClause->data[1]);
