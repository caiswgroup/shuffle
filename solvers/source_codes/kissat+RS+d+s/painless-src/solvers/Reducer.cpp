...
void 
Reducer::addOriginClauses(simplify *S) {
   return solver->addOriginClauses(S);
}
...
void 
Reducer::setParameter(parameter p)
{

}
void 
Reducer::initshuffle(int id)
{
                               ...
                               ClauseExchange ** outCls)
{
   ...
   if (res == UNSAT) {
      ...
      // if (sattime == 0 && cls->size <= 3) {
      //    ++sattime;
      //    printf("c strengthed get sat assume: ");
      //    for (size_t ind = 0; ind < cls->size; ind++) 
      //       printf("%d ", cls->lits[ind]);
      //    printf("0\t\t\t");
      //    for (size_t ind = 0; ind < tmpNewClause.size(); ind++) 
      //       printf("%d ", tmpNewClause[ind]);
      //    printf("0\n");
...
void
Reducer::addClauses(const std::vector<ClauseExchange *> & clauses)
{
   solver->addClauses(clauses);
}

void
Reducer::addInitialClauses(const std::vector<ClauseExchange *> & clauses)
{
   solver->addInitialClauses(clauses);
}

void
Reducer::addLearnedClauses(const std::vector<ClauseExchange *> & clauses)
{
...
void
Reducer::getLearnedClauses(std::vector<ClauseExchange *> & clauses)
{
