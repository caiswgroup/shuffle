...
int main(int argc, char ** argv)
{
   ...
   if (Parameters::getBoolParam("simp")) {
      printf("c Use simplify\n");
      S = new simplify();
      S->read_var_clause_num(Parameters::getFilename());
      ...
      if (S->clauses<=1e7/S->vars) {
         int res = S->simplify_card();
         if (!res) {
      ...
      if (S->clauses <= 15000000)
         S->simplify_resolution();
      // S->simplify_easy_clause();
      if (S->clauses <= 15000000)
         S->simplify_binary();
      // S->print_cnf("edit.txt");
   ...
   //    printf("c id: %d, var: %d\n", id, solvers[id]->getVariablesCount());
   //    if (id % 2) {
