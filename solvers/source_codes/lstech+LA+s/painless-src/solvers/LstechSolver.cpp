...
void cbkLstechExportClause(void * issuer, int lbd, vec<Lit> & cls)
{
   ...
   if (mp->assume_var) add++;
	...
	ClauseExchange * ncls = ClauseManager::allocClause(cls.size() + add);

	for (int i = 0; i < cls.size(); i++) {
		ncls->lits[i] = INT_LIT(cls[i]);
	}
   if (add) ncls->lits[cls.size()] = -mp->assume_var;
...
void 
LstechSolver::addOriginClauses(pakissat *P, int v) {
      // printf("YES\n");
   assume_var = v; 
